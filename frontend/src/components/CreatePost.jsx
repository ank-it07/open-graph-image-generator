...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Generate OG image
    const { data } = await axios.post('http://localhost:3000/generate-og-image', { ...postData, id: postId });
  ...
  const savePostToBackend = async (postData) => {
    ...
    const response = await axios.post('http://localhost:4000/posts', postData); // Adjust the URL to your backend
  ...
  const updatePostWithImageUrl = async (postId, imageUrl) => {
    ...
    await axios.put(`http://localhost:4000/posts/${postId}`, { ogImageUrl: imageUrl });
  };
